...
//        NSLog(@"表已经存！");
        ...
        NSString *sql = [NSString stringWithFormat:@"create table IF NOT EXISTS %@ ('%@' text,'%@' text,'%@' text,'%@' integer,'%@' text)",tableName,kBeanIdKey,k_small_BigId,k_small_name,k_small_location,k_small_sync];
...
- (id)mappingRs2Bean:(FMResultSet *)rs {
    ...
    bean.sync = [rs stringForColumn:k_small_sync];
    return bean;
}
